...
let SLDEditor = class SLDEditor extends LitElement {
    ...
    renderContainer(bayOrVL, preview = false) {
        ...
        if (!this.placing && !this.resizing && !this.connecting) {
            moveHandle = svg `
<a class="handle" href="#0" xlink:href="#0" @click=${() => this.dispatchEvent(newStartPlaceEvent(bayOrVL))}>
  <svg xmlns="${svgNs}" height="1" width="1" fill="black" opacity="0.83"
    ...
    `;
            resizeHandle = svg `
<a class="handle" href="#0" xlink:href="#0" @click=${() => this.dispatchEvent(newStartResizeEvent(bayOrVL))}>
  <svg xmlns="${svgNs}" height="1" width="1" fill="black" opacity="0.83"
